...
int mecab_do3(char *args, std::stringstream *sout, std::stringstream *serr) {
#define WHAT_ERROR(msg) do {                    \
    *sout << msg << std::endl;              \
    return EXIT_FAILURE; }                      \
  while (0);
  ...
  if (param.get<bool>("help")) {
    *sout << param.help() << std::endl;
    return EXIT_SUCCESS;
  }

  if (param.get<bool>("version")) {
    *sout << param.version() << std::endl;
    return EXIT_SUCCESS;
  }
  ...
  if (param.get<int>("lattice-level") >= 1) {
    *serr << "lattice-level is DEPERCATED. "
              << "use --marginal or --nbest." << std::endl;
  }

  MeCab::scoped_ptr<MeCab::ModelImpl> model(new MeCab::ModelImpl);
  if (!model->open(param)) {
    *sout << MeCab::getLastError() << std::endl;
    return EXIT_FAILURE;
  }
  ...
  if (param.get<bool>("dictionary-info")) {
    for (const MeCab::DictionaryInfo *d = model->dictionary_info();
         d; d = d->next) {
      *sout << "filename:\t" << d->filename << std::endl;
      *sout << "version:\t" << d->version << std::endl;
      *sout << "charset:\t" << d->charset << std::endl;
      *sout << "type:\t" << d->type   << std::endl;
      *sout << "size:\t" << d->size << std::endl;
      *sout << "left size:\t" << d->lsize << std::endl;
      *sout << "right size:\t" << d->rsize << std::endl;
      *sout << std::endl;
    }
    return EXIT_FAILURE;
  }
